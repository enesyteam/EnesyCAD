...
namespace DemandLoading
{
    public class RegistryUpdate
    {
        ...
        private static void CreateDemandLoadingEntries(string name, string path, List<string> globCmds, List<string> locCmds, List<string> groups, int flags, bool currentUser)
        {
                ...
                using (appk)
                {
                    ...
                    using (rk)
                    {
                        ...
                        if ((globCmds.Count == locCmds.Count) && globCmds.Count > 0)
                        {
                            ...
                            using (ck)
                            {
                                for (int i = 0; i <= globCmds.Count - 1; i++)
                                {
