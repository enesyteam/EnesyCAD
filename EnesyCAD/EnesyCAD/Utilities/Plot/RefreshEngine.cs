...
namespace EnesyCAD.Utilities.Plot
{
    class RefreshEngine
    {
        ...
        private System.Data.DataTable RefreshBlock(Document doc, Database db, Editor ed)
        {
            ...
            distAndAngle[1] = Utilities.AngleFromXAxisInXYPlane(origin, minPoint);
            distAndAngle[2] = origin.DistanceTo(maxPoint);
            distAndAngle[3] = Utilities.AngleFromXAxisInXYPlane(origin, maxPoint);
            ...
            foreach (BlockTableRecord blkTabRec in blkTabRecs)
            {
                ...
                minPoint = Utilities.Polar(origin, distAndAngle[0], distAndAngle[1]);
                maxPoint = Utilities.Polar(origin, distAndAngle[2], distAndAngle[3]);
                int i = 0;
        ...
        private string GetBlockChar(Document doc)
        {
            ...
            BlockNameType nType = BlockNameType.Name;
            if (m_rdoFileName.Checked) nType = BlockNameType.FileName;
            if (m_rdoPath.Checked) nType = BlockNameType.Path;

            switch (nType)
            {
                case BlockNameType.Name:
                    n = FrameBlock.Name;
                    break;
                case BlockNameType.FileName:
                    n = Path.GetFileName(FrameBlock.PathName);
                    break;
                case BlockNameType.Path:
